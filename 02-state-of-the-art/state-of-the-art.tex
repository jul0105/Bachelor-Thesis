...
\paragraph{\writingNotes{Register}} \label{sec:opaque_register}
...
% TODO TLS channel ???
% TODO register on the server vs register client-side


Client sends password to server via a secure channel. Server generate the salt and compute OPRF function with password and salt.
Server also generate 2 privates keys (one for the client and one for the server) and their corresponding public key. He encrypt client's private key and server's public key with OPRF output as a key and store the ciphertext alongside with his keys.

"Password registration is the only part of the protocol
...
avoid accidental storage of plaintext passwords that has affected also securityconscious companies [2, 3]. Note that this prevents S from checking password
...
(1) Optionaly, an OPRF can be used to archive Strong aPAKE following the aPAKE to SaPAKE compiler using OPRF from \cite{OPAQUE_Paper}. The OPRF take the client's password and server's salt as an input. Client use the output in place of his password for the rest of the protocol.
...
When the client want to login, the server sends its encrytped credentials and the client use it's password to decrypt the credentials (with or without OPRF). Then he can us his credentials to compute a Key-Hiding AKE with the server.

Both party finish with a mutual key confirmation initiated by the client.

\paragraph{\writingNotes{Register}}
Same problem addressed in \ref{sec:opaque_register} (client need to sends password to server-side register OR server send his public key for client-side register)
...
\begin{center}
   \begin{tabular}{ | c | p{8cm} || p{1cm} | p{1cm} | p{2cm} | p{2cm} | }
     ...
     1 & Avoid sending cleartext password to the server & x & x & Yes (register?) & Yes (register?) \\ \hline
     2 & Secure against pre-computation attacks & x & x & Yes & Yes, if using OPRF \\ \hline
     3 & Server doesn't send salt in cleartext & x & x & Yes & -- (no salt) \\ \hline
     4 & Forward secrecy & x & x & Yes, Full FS & Yes, Full FS \\ \hline
     5 & Mutual authentication & x & x & Yes & Yes ? \\ \hline
     6 & PKI-free & x & x & Yes, except during register (client-side register ?) & Yes, except during register (client-side register ?) \\ \hline
