...
\begin{center}
   \begin{tabular}{ | c | p{8cm} || p{1cm} | p{1cm} | p{2cm} | p{2cm} | }
     ...
     8 & Built-in mechanism to store client's secrets server-side & x & x & Yes & x \\ \hline
     9 & Server threshold implementation & x & x & Yes, user-transparent & x \\ \hline
     11 & Resistant to Oblivious PRF compromise & x & x & No, entire security is compromised & Fall back to non-strong aPAKE \\ \hline
...
\begin{enumerate}
 ...
 \item User can use password hardening technique to "increase the cost of a dictionary attack in case of server compromise". This is done by using ressource-heavy functions such as Scrypt \cite{Script_Paper} or Argon2 \cite{Argon2_Paper} instead of computing a simple and efficient hash. These functions allows to drastically slows down hashing process and so making offline attacks and online guessing attack much slower.
 \item OPAQUE "has a built-in facility for password-based storage-and-retrieval of secrets and credentials"
 \item OPAQUE "accommodates a user-transparent server-side threshold implementation"
 \item x
 \item ``There is a significant difference in the reliance on the security of
...
quantum-safe aPAKEs based on KHAPE with key hiding quantum-safe KEMs.'' \cite{OPAQUE_Paper}.
 \item x
 \item OPAQUE "security proof in a very strong model"
 \item x
 \item x
 \item x
\end{enumerate}
