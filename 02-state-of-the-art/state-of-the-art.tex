...
The figure \ref{fig:EKE_DH} shows the EKE protocol using DH during login process.
...
The figure \ref{fig:OPAQUE_AKE} shows the OPAQUE protocol using OPRF and AKE during login process.
The steps are the following :
...
(7) If the password entered is correct, client uses $sk$ to decrypt $ek$ and retrieve his private key $privU$.
...
He then uses these keys to run an authenticated key exchange with the server (like HMQV ?).
...
(1) Optionally, an OPRF can be used to archive Strong aPAKE following the aPAKE to SaPAKE compiler using OPRF from \cite{OPAQUE_Paper}. The OPRF takes the client's password and server's salt as an input. Client uses the output in place of his password for the rest of the protocol.
...
\begin{center}
   \begin{tabular}{ | c | p{8cm} || p{1cm} | p{1cm} | p{2cm} | p{2cm} | }
     ...
     10 & Server doesn't store passwords in cleartext & No & x & Yes & Yes \\ \hline
